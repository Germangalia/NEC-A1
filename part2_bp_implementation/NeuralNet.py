...
class NeuralNet:
    ...
    def fit(self, X, y, epochs=100, validation_split=0.0):
        ...
        # Ensure X and y have at least 2 dimensions: (n_samples, n_features)
        # This enforces the Part 2 requirement that the code receives a dataset
        # with the general structure (patterns x features).
        if X.ndim == 1:
            X = X.reshape(-1, 1)
        if y.ndim == 1:
            y = y.reshape(-1, 1)
        if X.ndim == 0 or y.ndim == 0:
            raise ValueError(
                "X and y must have at least 1 dimension: "
